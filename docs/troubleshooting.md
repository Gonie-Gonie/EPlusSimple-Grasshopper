...
For a source checkout, rerun `setup.cmd` after installing Rhino, then run
`build.cmd`. Generated host logs are below `temp\grasshopper-smoke`; stable
...
`scripts\clean.ps1` removes disposable `temp` output and generated artifacts
