...
_DOTNET_PROJECT = re.compile(
    r"^Dragons\.(?:InvisibleDragon|SimpleDragon)(?:\.[A-Za-z][A-Za-z0-9]*)*$"
)
_DOTNET_SYMBOL = re.compile(r"^Dragons\.(?:InvisibleDragon|SimpleDragon)(?:\.|$)")
...
def _load_lock(path: Path) -> UpstreamLock:
    ...
    if schema != "dragons.upstream-lock.v1":
        raise ConfigurationError(
            f"upstream lock.schema must be 'dragons.upstream-lock.v1', found '{schema}'"
        )
...
def _load_port_map(path: Path) -> tuple[PortMapping, ...]:
    ...
    for index, raw in enumerate(values):
        ...
        if _DOTNET_PROJECT.fullmatch(project) is None:
            raise ConfigurationError(
                f"{context}.dotnet.project must use an InvisibleDragon or SimpleDragon name"
            )
        dotnet_symbol = _text(dotnet["symbol"], f"{context}.dotnet.symbol")
        if _DOTNET_SYMBOL.match(dotnet_symbol) is None:
            raise ConfigurationError(
                f"{context}.dotnet.symbol must use an InvisibleDragon or SimpleDragon namespace"
            )
        tests = tuple(
