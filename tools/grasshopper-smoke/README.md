...
```powershell
tools\grasshopper-smoke\run.cmd -Host Rhino8 -Scenario InvisibleOnly
tools\grasshopper-smoke\run.cmd -Host Rhino7 -Scenario SimpleOnly
tools\grasshopper-smoke\run.cmd -Scenario All
```
...
```powershell
package.cmd
tools\grasshopper-smoke\run.cmd -Source PortablePackage -Scenario All -Target All -SkipPluginBuild
```

`package.cmd -RunPortableHostGate` performs those two steps as the opt-in local
...
```powershell
tools\grasshopper-smoke\run.cmd -ArchiveSafetySelfTest
```
