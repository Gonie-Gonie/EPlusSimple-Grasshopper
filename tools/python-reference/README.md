# Python reference oracle

The released Grasshopper plugins do not depend on Python. This tool runs the
historical `epsimple` and `idragon` implementation only as a behavioral oracle
for the C# port.

`.\dev.cmd reference` verifies the exact Python 3.12.7 interpreter selected by
`.\dev.cmd setup`, materializes the commit in `upstream/upstream.lock.json`, installs
the exact requirements into `.tools/python-reference`, and writes all generated
work to `temp/reference/python-output`.

The requirements file is a complete `--no-deps` closure. Every run verifies
that the repo-local directory contains exactly those pinned distributions and
that all active package metadata requirements are present and compatible; a
global Python installation cannot satisfy a missing oracle dependency.
Oracle, compatibility, and nested regeneration processes also disable Python's
automatic site loading and unsafe path prefix, clear inherited `PYTHONHOME` and
`PYTHONPATH`, and preserve the pinned `PYTHONHASHSEED=0` behavior.

```text
.\dev.cmd reference
.\dev.cmd reference -Mode Verify
.\dev.cmd reference -RefreshDependencies
```

The generators fix `PYTHONHASHSEED`, remove CPython memory addresses from the
generated IDF with a stable first-occurrence mapping, and record hashes for
every output. The construction equality/hash oracle additionally binds its
Material, Layer, and Construction observations to the exact public-symbol
inventory, source bytes, and five upstream symbol hashes. The ScheduleType
oracle binds all 12 upstream ScheduleType symbols and records the four exact
five-field `ScheduleTypeLimits` objects plus 44 boundary, coercion, type-error,
and tagged non-finite validation cases. Its `real` NaN and infinity results use
strict JSON tags instead of non-standard numeric tokens. The DaySchedule
core oracle binds the remaining 14 lifecycle, immutable-update, factory,
compactization, summary, time-grid, IDF-expression, and type symbols through
42 exactly ordered cases. It preserves recursive binary64 hex facts and
runtime-name policy tokens while pinning the exact four equivalent versus ten
reviewed native-adaptation split. The DaySchedule
metrics oracle binds 13 fixed-grid properties and includes a catastrophic
cancellation case that locks the CPython 3.12 compensated-float summation
behavior used by `average`, `integral`, and `positive_average`. The
DaySchedule, RuleSet, and Schedule operation oracles each bind exactly 28
upstream symbols and preserve Python scalar kinds, reverse-operation names,
typed failures, tagged non-finite results, nested fallback topology, and source
immutability. A separate RuleSet core oracle binds the remaining 24 lifecycle,
immutable-slot, factory, lookup, summary, dictionary, IDF-expression, extrema,
and type symbols through 72 exactly ordered cases. It preserves default and
override alias topology, string and positive/negative integer day lookup,
fallback suppression, exact dictionary order, binary64 values, runtime-name
policy tokens, and the exact seven equivalent versus seventeen reviewed
native-adaptation split. The annual Schedule corpus additionally records all
365 days and the exact inclusive compact-period union across asymmetric
operands. A separate Schedule core oracle binds the remaining 22 annual
lifecycle, factory, metric,
summary, IDF, and compact-unification symbols through 104 cases. It records
case-local alias graphs, input postconditions, CPython 3.12 binary64 results,
runtime-name normalization, partial-mutation defects, read-only day-list and
contiguous IDF-field native mappings, and the exact 12 equivalent versus 10
reviewed native-adaptation split. A final profile residual oracle binds the
five remaining public `profile.py` symbols through 15 ordered cases. It pins
Profile slot and IDF-export order, Schedule container topology, and the native
schedule-operation exception family as one equivalent behavior and four
reviewed adaptations. The usage-profile core oracle binds the 13 in-scope
public symbols in `epsimple/core/profile.py` through 39 ordered cases. It pins
the exact 14-key dictionary contract, standard and extended database lookup,
invariant CSV parsing, deterministic IDs, and the complete seven-schedule
conversion for all 24 embedded profiles, including vacation and holiday day
topology. The utils core oracle binds the exact two historical `utils.py`
sources and their four public-symbol receipts through 12 ordered cases. It
records the legacy GRJSON template's exact order, defaults, deep-copy
isolation, and shared-global mutation behavior together with the Python
type, range, and enum decorator surfaces, while declaring the four reviewed
immutable or strongly typed native adaptations explicitly. `-Mode Verify`
compares all generated files byte-for-byte with the reviewed baseline under
`fixtures/reference/python-0.7.0`. Every reference run also executes the
fail-closed generator tests under `tests/PythonReference` before producing an
oracle.

The common core oracle directly imports the pinned `idragon/common.py` bytes
without initializing the surrounding package and binds its 13 unresolved
symbols through 39 ordered cases. It preserves the 24.2.0 and 2026 defaults,
two- and three-component construction, arbitrary nonnumeric delimiters,
Unicode decimal digits, the empty-format separator quirk, ordered iteration,
legacy IDD and EnergyPlus directory names, and the upstream invalid-input
failure surface. Its consumer contract declares ten exact equivalents and the
three reviewed native descriptor, validated-construction, and strongly typed
coercion adaptations.

The constants engineering oracle binds the eight `THERMAL` and `Unit` symbols
in the exact pinned `idragon/constants.py` source through 24 ordered cases. It
records canonical binary64 conversion values and engineering probes, the
five-name versus three-iterated-member `Unit` alias topology, and the 107 W
people-activity value emitted by the default IDF. The two mutable Python enum
containers are reviewed native adaptations; all six engineering members are
exact equivalents. Package directories and Python package metadata are not
part of this engineering corpus.

The epsimple numeric-constants oracle binds the 29 `ConvectionHeatTransfer`,
`Site2CO2`, `Site2Cost`, `Site2Source`, and `Unit` symbols in the exact pinned
`epsimple/constants.py` source through 87 ordered cases. It records each
binary64 declaration and engineering probe, float-enum construction and member
topology, the `Unit.MM_TO_M`/`W_TO_KW` alias, and the three-name 1.1
`Site2Source` alias. The latter also pins the historical result-scaling split:
GRR constants expose all five declarations while enum iteration supplies only
three zipped factors and leaves the final two carrier rows unscaled. That split
is observed by compiling and directly executing the exact pinned
`GreenRetrofitResult.to_source_uses` AST from `core/model.py`; the model file,
AST, method, and `VALID_DIGITS` inventory receipts are fail-closed provenance.
Five enum containers are reviewed native dispatch/container adaptations; all
24 numeric members are exact equivalents.

The epsimple construction-core oracle binds exactly 48 unresolved class,
constructor, property, conversion, database, regulation, and special-state
targets in the pinned `epsimple/core/construction.py` source through 19 ordered
CPython 3.12.7 cases. It executes material, fenestration, and surface
construction state and validation; ID generation; `from_json`, `to_dict`, and
`to_dragon`; database load/get; U-value, depth, heat-capacity, and unique-
material derivation; regulation selection; simple construction creation and
reversal; and Special/Open/Unknown singleton behavior. Isolated package imports
and a byte-identical relocated `epsimple`/`idragon` source copy pin path-
independent behavior. The consumer contract declares seven exact equivalents
and 41 explicit native-route exceptions with stable adaptation IDs. The 12
adjacent equality, hash, representation, and string declarations remain
explicitly excluded and cannot be promoted by these cases. It is generated by
`generate_epsimple_construction_core_oracle.py`.

The epsimple HVAC enum/base oracle binds exactly 28 `CompressorType`,
`CoolingTowerControl`, `CoolingTowerType`, `Fuel`, `NoneSource`, and
`SourceSystem` declarations in the pinned `epsimple/core/hvac.py` source
through six ordered CPython 3.12.7 cases. It executes enum topology, values,
strings, lookup and dragon conversion; the NoneSource singleton, ID and null
conversion; and the empty base-class and mutable type-mapper behavior with
error boundaries. Isolated imports plus a byte-identical relocated source copy
prove path-independent behavior. The consumer contract declares 18 exact
equivalents and ten reviewed CLR enum, nullable source, sealed aggregate, or
GRM dispatch adaptations. The remaining source partition is fail-closed as 116
deferred declarations plus 58 explicit exclusions. It is generated by
`generate_epsimple_hvac_enums_base_oracle.py`.

The epsimple HVAC other-systems oracle binds exactly 17 declarations at
inventory indices `283,284,287,290-295;325,326,329,332-336` in the pinned
`epsimple/core/hvac.py` source. Two ordered CPython 3.12.7 cases execute the
`PhotoVoltaicSystem` and `VentilationSystem` constructors, process-identity
default IDs, names, defaults, explicit and mutated state, property validation
boundaries, `from_json`, and fresh `to_dragon` conversion. The observations
preserve upstream acceptance of selected bool, NaN, infinity, and blank-name
inputs and the legacy ventilation conversion that omits airflow. Isolated
imports plus a byte-identical relocated source copy prove path independence.
The consumer contract declares nine direct ID/property equivalents and eight
reviewed immutable-aggregate, GRM reader/writer, or GreenRetrofit-conversion
exceptions with stable adaptation IDs. All 185 adjacent HVAC declarations are
pinned as a fail-closed non-target closure and cannot be promoted by these
cases. It is generated by `generate_epsimple_hvac_other_systems_oracle.py`.

The epsimple HVAC thermal-source oracle binds exactly 47 declarations at
inventory indices `135,136,139,142-146;157,158,161,164-169;170,171,174,177-184;
199,200,203,206-208;248,251,252;253,254,257,260-266` in the pinned
`epsimple/core/hvac.py` source. Six ordered CPython 3.12.7 cases execute the
`AbsorptionChiller`, `Boiler`, `Chiller`, `DistrictHeating`,
`GeothermalHeatPump`, and `HeatPump` constructors, defaults, explicit and
mutated state, property validation, `from_json`, and recursive `to_dragon`
behavior, including all four chiller cooling-tower type/control branches and
fresh conversion outputs. Isolated imports plus a byte-identical relocated
source copy prove path independence. The consumer contract declares 24 direct
ID/property equivalents and 23 reviewed sealed `SourceSystem`, GRM-dispatch,
and GreenRetrofit-conversion exceptions with stable adaptation IDs. All 155
adjacent HVAC declarations are pinned as a fail-closed non-target closure and
cannot be promoted by these cases. It is generated by
`generate_epsimple_hvac_thermal_source_oracle.py`.

The epsimple HVAC supply-system oracle binds exactly 52 declarations at
inventory indices `147,148,151,154-156;209,210,213,216-220,223,226-231,234,
237-239;271,272,275,278-282;296,297,300,303-309,312,315-318;321-324` in the
pinned `epsimple/core/hvac.py` source. Eight ordered CPython 3.12.7 cases
execute the `AirHandlingUnit`, `ElectricRadiantFloor`, `ElectricRadiator`,
`FanCoilUnit`, `PackagedAirConditioner`, `RadiantFloor`, `Radiator`, and
`SupplySystem` constructors, defaults, explicit and mutated state, source-
system validation, `from_json`, and recursive `to_dragon` behavior, including
the packaged-air-conditioner dedicated source map and the mutable supply-
system type mapper. Isolated imports plus a byte-identical relocated source
copy prove path independence. The consumer contract declares 19 direct
equivalents and 33 reviewed sealed aggregate, nullable source, GRM-dispatch,
or GreenRetrofit-conversion exceptions with stable adaptation IDs. All 150
adjacent HVAC declarations are pinned as a fail-closed non-target closure and
cannot be promoted by these cases. It is generated by
`generate_epsimple_hvac_supply_system_oracle.py`.

The epsimple identifier-conventions oracle binds the 34 unresolved
`AUTOID_PREFIX`, `Directory`, `PackageInfo`, and `SpecialTag` symbols in the
exact pinned `epsimple/constants.py` source through 22 ordered cases. It pins
enum declaration and iteration topology, string and formatting behavior,
construction and lookup failures, mutation and copy aliasing, relocatable
resource-path roles, package metadata operations, and class-versus-instance
attribute semantics. The consumer contract declares 23 exact equivalents and
11 reviewed immutable, embedded-resource, caller-supplied-resource, package-
metadata, or compiled-target adaptations. The two `__repr__` declarations
remain explicitly outside this bounded corpus. It is generated by
`generate_epsimple_identifier_conventions_oracle.py`.

The epsimple model-core oracle binds exactly 35 unresolved table, error,
GreenRetrofitModel, and address-to-weather targets in the pinned
`epsimple/core/model.py` source through 11 ordered CPython 3.12.7 cases. It
executes weather tables and lookup, EnergyPlusError state, model construction
and validation, area/exterior projections, six weighted averages and zero
boundaries, source-system and unique-catalog behavior, full temporary GRM graph
loading, adjacency, `to_dragon`, `to_idf`, and an instrumented `run` success and
failure path without launching EnergyPlus. Isolated imports plus a byte-
identical relocated source copy prove path independence. The consumer contract
declares 11 exact equivalents and 24 explicit native-route exceptions. Three
representation/Excel declarations stay out of scope, while all 14
GreenRetrofitResult declarations are deliberately deferred to a separate
bounded corpus. It is generated by `generate_epsimple_model_core_oracle.py`.

The epsimple model-result oracle binds exactly the 14 `GreenRetrofitResult`
declarations at inventory indices 373 through 386 in the pinned
`epsimple/core/model.py` source through 11 ordered CPython 3.12.7 cases. It
executes result construction, area and rounding, domestic-hot-water demand and
server selection, site/source energy, carbon and cost factors, gross and
per-area summaries, dictionary topology, and deterministic JSON file writing
with overwrite and failure boundaries. Isolated imports plus a byte-identical
relocated source copy prove path-independent behavior. The consumer contract
declares nine exact equivalents and five reviewed immutable result, validated
builder, typed DHW, or deterministic writer adaptations, while binding the
other 38 `model.py` declarations as adjacent non-target receipts. It is
generated by `generate_epsimple_model_result_oracle.py`.

The epsimple shape-core oracle binds exactly 53 unresolved area-based geometry
targets in the pinned `epsimple/core/shape.py` source through 17 ordered
CPython 3.12.7 cases. It executes BlindType, Fenestration, Door, GlassDoor,
Window, Surface, and Zone construction, validation, factory/JSON dispatch,
copy and flip behavior, construction aggregation, opening counts, supply-system
filtering, and native dragon conversions or upstream failures. Isolated imports
plus a byte-identical relocated `epsimple`/`idragon` source copy prove path-
independent behavior. The consumer contract declares 33 exact equivalents and
20 explicit native-route exceptions with stable adaptation IDs. The five
adjacent representation/equality declarations remain explicitly excluded from
this bounded corpus. It is generated by
`generate_epsimple_shape_core_oracle.py`.

The dragon construction core oracle binds exactly 35 direct-state class,
constructor, property, enum, and reversal targets across `Construction`,
`Glazing`, `Layer`, `Material`, `MaterialRoughness`, and `NoMassConstruction`
through 19 ordered CPython 3.12.7 cases. It records default, explicit, and
mutated state; bool, range, and nonfinite boundaries plus error timing; exact
binary64 derived-property operation order; and child/container alias topology.
Eleven targets are exact equivalents and 24 use reviewed native adaptations.
`Construction.reversed` is an adaptation because the native model validates
names and copies only the container around shared immutable `Layer` references,
while the upstream result shares mutable `Layer` objects.
AirBoundary, representation strings, equality/hash behavior, and all IDF
emission are explicitly outside this direct-state corpus. It is generated by
`generate_dragon_construction_core_oracle.py`.

The dragon construction-family-to-IDF-object oracle binds two common-valid-
state cases each for `AirBoundary.to_idf_object`, `Construction.to_idf_object`,
`Glazing.to_idf_object`, `Layer.to_idf_object`, and
`NoMassConstruction.to_idf_object`. It records every allowed field in order,
fresh returned IDF objects, fresh lists where applicable, and unchanged source
state. Construction
additionally binds its surface argument and the resulting
`<construction>:for:<surface>` name linkage. The separate reviewed
`model-context-air-boundary-idf-emission`,
`model-context-construction-idf-emission`,
`model-context-glazing-idf-emission`, `model-context-layer-idf-emission`, and
`model-context-no-mass-construction-idf-emission` adaptations target the
relevant private `EnergyModelIdfAssembler` paths through
`EnergyModel.ToIdfDocument`. Constructors and properties, equality/hash
behavior, invalid-domain and error semantics, standalone `IdfObject` policy,
parent Surface/Zone/model emission, and native deduplication, conflict,
compaction, and global ordering stay unresolved. It is generated by
`generate_dragon_construction_to_idf_object_oracle.py`.

The dragon-HVAC photovoltaic-to-IDF-object oracle binds three common-valid-
domain cases for `PhotoVoltaicPanel.to_idf_object`. It records the exact
six-object EnergyPlus family sequence and complete allowed fields, fresh result
and object topology, `sqrt(area)` panel dimensions, default and custom
effective-area ratios, cross-object names, tilt/azimuth boundaries, and unit
efficiency/ratio values. The reviewed
`compact-native-photovoltaic-idf-emission` mapping targets
`PhotovoltaicPanel.ToIdfObjects`; its compact native representation preserves
populated and default semantics while omitting trailing blank/default fields.
The class and constructor, property-validation order and errors, invalid or
nonfinite domain state, isolated `IdfObject` policy, and parent
`EnergyModel.to_idf` remain unresolved. It is generated by
`generate_dragon_hvac_photovoltaic_to_idf_object_oracle.py`.

The dragon-HVAC appenders/controllers oracle binds six deterministic cases to
24 exact inventory receipts for DemandBranchAppender, EquipmentListAppender,
SequentialLoadFractionController, SupplySystemToIdfPostProcessor,
ZoneAirNodeAppender, and ZoneTerminalUnitAppender. Relocated upstream imports
run against bounded IDF stubs to record branch-list/splitter/mixer append order,
equipment counts and the 99-entry limit, absent/existing NodeList handling,
terminal-list failures, sequential target lookup and overflow, zero/one/multi
active fraction schedules, ALLOFF and epsilon arithmetic, exact failure
prefixes, and repeated-run mutation. The reviewed public InvisibleDragon route
is SupplyGroup through ZoneHvacAssignment and EnergyModel.ToIdfDocument to the
resulting IdfDocument, so all 24 internal-behavior receipts remain conservative
exceptions rather than standalone native-route claims. SupplyGroup conversion
receipt 796 is immutable hash-pinned support; 149 declarations remain deferred,
closing the exact 174-receipt source partition. The generator performs no
native or EnergyPlus process execution. It is generated by
`generate_dragon_hvac_appenders_controllers_oracle.py`.

The dragon-HVAC misc-systems core oracle binds six deterministic cases to the
exact 15 `DomesticHotWater`, `EnergyRecoveryVentilator`, and
`PhotoVoltaicPanel` class, constructor, property, and empty-emission receipts.
Byte-identically relocated imports record constructor and setter validation,
mutation, subnormal and upper boundaries, tagged nonfinite values, bool-as-int
quirks, permissive ERV state, photovoltaic `azimuth=None`, and fresh empty
DHW/ERV result lists. Reviewed public native routes yield seven equivalents and
eight conservative exceptions; DomesticHotWater uses its direct public API,
ERV emission uses the public EnergyModel aggregate, and photovoltaic properties
use the public panel API. Index 761 remains immutable photovoltaic-to-IDF
support. Together with the other three active HVAC corpora, 21 resolved and six
out-of-scope declarations, the generator fail-closes an exact disjoint
174-receipt source partition without running native code or EnergyPlus. It is
generated by `generate_dragon_hvac_misc_systems_core_oracle.py`.

The dragon-HVAC source/tower core oracle binds ten deterministic cases to 59
exact inventory receipts. It executes relocated upstream imports to observe
`Fuel` and `CompressorType` ordering/value/lookup behavior, source class and
constructor topology, HeatPump/GeothermalHeatPump, boiler/chiller variants,
four concrete cooling towers, public name derivations, and capacity-selection
branches. The observations intentionally retain legacy permissive validation,
fuel coercion, abstract-class, screw-compressor type, mutable-name, and
closed-two-speed capacity-omission quirks. Reviewed public InvisibleDragon
routes support 27 equivalents and 32 conservative exceptions. The generator
hash-pins 13 adjacent conversion receipts to the existing source-system-to-IDF
oracle and keeps only `CompressorType.__str__` and `Fuel.__str__` out of scope,
providing an exact 74-receipt family closure without executing native or
EnergyPlus processes. It is generated by
`generate_dragon_hvac_source_tower_core_oracle.py`.

The dragon-HVAC supply-core oracle binds nine deterministic cases to the exact
49 target receipts for `AirHandlingUnit`, both electric radiant systems,
`FanCoilUnit`, `PackagedAirConditioner`, both hydronic radiant systems,
`SupplyGroup`, and `SupplySystem`. Isolated and byte-identically relocated
imports execute constructor state, source-dependent heating/cooling capability,
public naming rules, concrete `to_idf_object` output order, availability
selection, validation quirks, deepcopy behavior, and fresh projection/result
containers. Eight already-classified adjacent receipts complete the exact
57-declaration family closure without being promoted. Reviewed public
InvisibleDragon routes support 18 equivalents and 31 conservative exceptions;
the corpus never claims internal `Generate` members. The generator also
immutably hash-pins the existing SupplyGroup core, SupplyGroup conversion, and
model add-supply-system fixtures as supporting evidence. It is generated by
`generate_dragon_hvac_supply_core_oracle.py`.

The dragon-HVAC SupplyGroup core oracle binds 18 cases, three each for
`SupplyGroup.__init__`, `coolable`, `cooling_systems`, `heatable`,
`heating_systems`, and `sources`. Logical probes capture exact constructor
validation, tuple snapshots, capability reads, valid-domain projection
order and identity, tuple freshness, static first-seen ordering across distinct
entity identifiers, and source identity de-duplication without serializing
runtime addresses. Constructor validation and stable native
entity-ID source de-duplication are explicit reviewed adaptations; the four
capability projections map to `CanCool`, `CoolingSystems`, `CanHeat`, and
`HeatingSystems`. The generator audits the complete twelve-module local import
graph. It deliberately excludes the `SupplyGroup` class receipt and
`to_idf_object`, which remain outside this core slice. It is generated by
`generate_dragon_hvac_supply_group_core_oracle.py`.

The dragon-HVAC SupplyGroup-to-IDF-object oracle binds three bounded
`SupplyGroup.to_idf_object` orchestration cases: ordered system/availability
zipping and capability reads, flattened child outputs with immediate custom
availability conversion and the sequential controller last, system failure,
and availability failure. Repeated success conversion records fresh result
lists, child lists, and controller identity without serializing runtime
addresses. It uses reviewed adaptation
`model-context-supply-group-idf-assembly`, public target
`EnergyModel.ToIdfDocument`, and assertion
`dragon-hvac-supply-group-to-idf-object-3f9c508c`. The standalone converter API,
`SupplySystem.to_idf_object`, `SourceSystem.to_idf_object`,
`SequentialLoadFractionController` and its `run` method, concrete converters,
arbitrary probe systems/schedules, and full `EnergyModel.to_idf` remain explicit
unresolved boundaries. It is generated by
`generate_dragon_hvac_supply_group_to_idf_object_oracle.py`.

The dragon-model add-supply-system oracle binds three bounded
`EnergyModel.add_supply_system` cases. It records generation failure before
mutation, processor failure after append mutation persists, exception prefixes,
and successful append/processor/`None` return order. The public native target is
`EnergyModel.ToIdfDocument`, using reviewed adaptation
`model-context-supply-system-assembly` and assertion
`dragon-model-add-supply-system-174532d0`. Its external temporary import audits
all twelve loaded local modules against exact source and Python 3.12 AST
receipts. `EnergyModel.to_idf`, `SupplyGroup`, concrete systems, and
postprocessors remain unresolved. It is generated by
`generate_dragon_model_add_supply_system_oracle.py`.

The dragon-model assembly oracle binds ten bounded `EnergyModel.to_idf` cases
against external temporary copies of the exact pinned source graph. Five model
cases record duplicate and case-only profile/schedule naming, ordered shared
unconditioned fallback objects, legacy ERV ventilation fields, an assigned
supply skipped because HVAC availability is absent, and exact raw default
geometry, people-activity, ALLON/ALLOFF, and schedule type-limit fields. Five
probe cases additionally record the complete parent orchestration order,
identity de-duplication, repeated projections, same-document return, and exact
layer/source/supply/PV failure prefixes. The generator fails unless all twelve
loaded sources match exact source and Python 3.12 AST receipts. The fixture
explicitly keeps `EnergyModel.to_idf` as `needs_reverification`: it has no
native adaptation, trusted assertion, child-converter closure, or full-symbol
closure claim. It is generated by `generate_dragon_model_assembly_oracle.py`.

The dragon-model class oracle targets only public-inventory index 815,
`EnergyModel`, through three CPython 3.12.7 cases: class and
`supported_versions` topology, shared-list append visibility with unconditional
`finally` restoration, and instance shadow/arbitrary-attribute/subclass
topology. It binds classification exception
`sealed-read-only-native-energy-model-class-a7582a41`, assertion
`dragon-model-energy-model-class-a7582a41`, and native type
`Dragons.InvisibleDragon.Model.EnergyModel`. Indices 816-825 are resolved
receipts only; the loaded `Version` symbols are context receipts only. The
external temporary import audits all twelve loaded local modules against exact
source and Python 3.12 AST receipts. Constructors and named members are not
separate targets or compatibility claims. It is generated by
`generate_dragon_model_class_oracle.py`.

The dragon-shape shading-material-to-IDF-object oracle binds three cases each
for `Blind.to_idf_object` and `Shade.to_idf_object`. It records their exact
EnergyPlus object types and complete ordered field sets, fresh result topology,
Shade emissivity arithmetic, permissive invalid numeric emission, and the
nonnumeric failure point. Constructor aliasing is context only. The reviewed
`model-context-shading-material-idf-assembly` mapping targets valid emission
through `EnergyModel.ToIdfDocument`; constructors, invalid native emission,
standalone conversion, shading controls, surfaces, and parent model assembly
remain unresolved. It is generated by
`generate_dragon_shape_shading_material_to_idf_object_oracle.py`.

The dragon-model conditioning oracle binds
`EnergyModel.conditioned_zones`, `EnergyModel.unconditioned_zones`, and
`Zone.is_conditioned` across the exact pinned `idragon/dragon/model.py` and
`idragon/dragon/shape.py` sources through nine ordered cases. It records zone
order, fresh-list and input-object identity semantics, falsey-but-present
profile availability, and the exact supply-plus-profile predicate using only
logical labels, indices, booleans, and tagged state. The two model list
properties are exact equivalents; the zone-local predicate is one reviewed
model-context adaptation. It is generated by
`generate_dragon_model_conditioning_oracle.py`.

The dragon-model construction-defaults oracle binds `EnergyModel.__init__`
and `EnergyModel.create_default_idf` in the exact pinned
`idragon/dragon/model.py` source through nine ordered cases. It records the
legacy constructor call shape, shared omitted-list defaults, explicit aliasing,
permissive assignments, and the fresh default IDF's exact 17-object family
order and raw fields. Default-IDF creation is an exact equivalent; the native
constructor is the reviewed
`immutable-validated-energy-model-construction` adaptation. It is generated by
`generate_dragon_model_construction_defaults_oracle.py`.

The dragon-model projections oracle binds `EnergyModel.surfaces`,
`EnergyModel.used_constructions`, `EnergyModel.used_layers`, and
`EnergyModel.used_profiles` through 12 ordered cases. It hashes the exact
`construction.py`, `model.py`, `profile.py`, and `shape.py` sources, records
CPython 3.12.7 seed-zero set iteration as reference data, and uses only logical
labels and registry indices for identity. AirBoundary and NoMass inputs also
pin their exclusion from opaque construction and layer projections. The surface and duplicate-name
profile projections are exact equivalents; native construction/layer results
retain the same membership and source objects in deterministic first-use order.
It is generated by `generate_dragon_model_projections_oracle.py`.

The dragon-model Terrain oracle binds the `Terrain` string-enum class and its
five public members in the exact pinned `idragon/dragon/model.py` source through
18 ordered cases. It records declaration and iteration order, exact title-case
EnergyPlus choice tokens, value/name construction, JSON projection, and the
pinned CPython qualified `Terrain.NAME` string that the historical IDF writer
emits. The class is one reviewed typed-native-enum adaptation that corrects the
rendered token for EnergyPlus; the five semantic member values are exact
equivalents.

The Imugi IDD-definitions core oracle binds eight relocatable CPython 3.12.7
cases to the exact 40 `IddField` and `IddObject` definition receipts in
`idragon/imugi.py`. Its consumer contract records 18 reviewed equivalents and
22 explicit exceptions, while 65 deferred and 28 out-of-scope declarations
close the exact disjoint 133-receipt source partition. Two byte-identical
relocated imports and the hash-pinned EnergyPlus 24.2 IDD schema support make
the corpus path-independent without running native code or EnergyPlus. It is
generated by `generate_imugi_idd_definitions_core_oracle.py`.

The remaining Imugi corpora close the same source partition without overlap.
`generate_imugi_idd_schema_static_core_oracle.py` binds 21 `IDD`, exception,
and `StaticIndexedDict` receipts through eight cases (nine equivalents and 12
exceptions). `generate_imugi_idf_object_core_oracle.py` binds 25 `IDF` and
`IdfObject` receipts through seven cases (six equivalents and 19 exceptions),
including the public `IdfDocument.Append` route. Finally,
`generate_imugi_idf_object_list_core_oracle.py` binds 19 `IdfObjectList`
receipts through five cases (four equivalents and 15 exceptions). Each corpus
uses relocated CPython observations, immutable public native-route metadata,
and fail-closed hash layers without executing native code or EnergyPlus.

The launcher result-parser oracle directly loads the pinned
`idragon/launcher.py` together with only its pinned sibling modules, without
executing `idragon.__init__`. Its 21 ordered cases bind the seven
`EnergyPlusResult` class, constructor, audit, boundary, error, ESO, and tabular
parser symbols. DataFrames are normalized as ordered columns, index, dtypes,
and tagged cells; NaN and finite binary64 values never enter JSON as
non-standard numeric tokens. The consumer contract records all seven reviewed
typed native adaptations, including `explicitly-unsupported-energyplus-eso`;
it does not claim that the native port parses ESO output.

The launcher runtime oracle directly loads the same pinned launcher source
under closed fakes. Its 12 ordered cases bind the runtime-not-found exception,
runtime discovery, scalar/broadcast dispatch, and single-run lifecycle through
exactly three cases per symbol. No process or active EnergyPlus load is
executed. Temporary work is confined to one unique controlled descendant, and
only logical path tokens enter the fixture. The consumer contract records four
reviewed structured runtime, verified discovery, bounded batch, and isolated
cancellable single-run adaptations.

Updating the tracked baseline is an explicit review action:

```text
.\dev.cmd reference -UpdateBaseline
```

For auditing an already available exact checkout without allowing the script
to change it, pass `-UpstreamPath <path>`. An explicit checkout must already be
clean, have the pinned origin, and be at the pinned commit.
