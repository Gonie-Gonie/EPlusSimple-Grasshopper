# Grasshopper examples and recipes

The repository includes two genuine binary Grasshopper starter definitions:

- `00-invisibledragon-material-construction.gh` connects Opaque Material to
  Layered Construction, supplies a thickness slider, and displays the computed
  U-value in a panel.
- `10-simpledragon-material-construction.gh` provides the equivalent
  SimpleDragon Material to SimpleDragon Surface Construction graph.

Both definitions contain public Dragon components and persisted Grasshopper
wires. They are generated by Rhino 7, then solved, saved, reopened, and
round-trip checked in headless Rhino 7 and Rhino 8 Grasshopper hosts. Run
`tools\example-definitions\run.cmd` to repeat validation, or add `-Generate` to
regenerate the canonical files before validating both hosts. All generated
candidates, round-trip copies, summaries, and logs stay below `temp/`.

The remaining examples are reproducible workflow recipes for broader graphs.
...
inputs, triggers, and persistence rules. Any additional binary release example
must pass the same Rhino 7 and Rhino 8 save/reopen gate before it is tracked.
