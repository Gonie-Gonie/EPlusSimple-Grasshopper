...
if ([string] $publicationSpec.projectLicense -cne 'MIT' -or
    ...
    throw 'Release publication metadata differs from the reviewed individual/MIT/support/provenance contract.'
}
...
if ([bool] $packagePublication.publicPublicationApprovedByOwner -ne [bool] $publicationSpec.publicPublicationApprovedByOwner -or
    ...
    [string] $packagePublication.weatherSource -cne [string] $publicationSpec.weatherSource) {
...
$releaseGate = [pscustomobject] [ordered] @{
    ...
    publication = [pscustomobject] [ordered] @{
        ...
        reason = 'This command creates a local candidate only; no tag, GitHub Release, Yak publication, or Food4Rhino submission has occurred.'
    }
}
